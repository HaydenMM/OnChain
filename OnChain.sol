...
contract MyEpicNFT is ERC721URIStorage {
	...
	uint256[] public numberArr = [0,0,0,1,2,0,1,0,1,2,0,1,1,2];
	function random(uint256 input, uint256[] memory arr) internal returns (uint256) {
		for (uint256 i = 0; i < arr.length; i++) {
			uint256 n = uint256(keccak256(abi.encodePacked(_tokenIds.current() + i))) % arr.length;
	...
	function generateFlowerNFT() public {
